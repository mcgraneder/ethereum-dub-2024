...
library Allowance {
  ...
  function updateAll(IWallet.PackedAllowance storage allowed, uint160 amount, uint48 expiration, uint48 nonce) internal {
    uint48 storedNonce;
    unchecked {
      storedNonce = nonce + 1;
    }
  ...
  function updateAmountAndExpiration(IWallet.PackedAllowance storage allowed, uint160 amount, uint48 expiration) internal {
